...
Each Dockerfile derives the immutable identity itself. Its first stage clones the build context's `.git` into a fresh working tree, which is clean by construction and contains only committed files, and runs `scripts/generate-deployment-identity.mjs` there. Later stages build only from the archive that stage produced and re-verify it. The build context should be a git checkout at the commit being deployed (a worktree's `.git` file does not work; build from a real clone). When the context has no `.git` at all, as on Render, the stage instead fetches exactly `PAGE2WEBMCP_GIT_COMMIT_SHA` from the public repository named by the `PAGE2WEBMCP_SOURCE_REPOSITORY` build argument (default `https://github.com/mikigraf/Page2WebMCP.git`); the commit, clean-tree, and archive checks are the same either way. Pass the same commit, release ID, and HTTPS control-plane origin to every image:
