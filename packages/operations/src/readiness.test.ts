...
test("only one exact selected-hash native installation proof constructs liveSuccess", () => {
  ...
  const mutations: Array<[string, Record<string, unknown>]> = [
    ...
    ["registration only", { installationProof: nativeProof({ authenticatedReadExecuted: undefined }) }],
    ["unconfirmed mutation", { installationProof: nativeProof({ confirmedReversibleMutationExecuted: false }) }],
    ["multiple mutation effects", { installationProof: nativeProof({ confirmedMutationEffectCount: 2 }) }],
    ["non-authoritative state", { installationProof: nativeProof({ authoritativeFinalStateVerified: false }) }],
    ["wrong execution tools", { installationProof: nativeProof({ executionToolsMatchCapabilities: false }) }],
