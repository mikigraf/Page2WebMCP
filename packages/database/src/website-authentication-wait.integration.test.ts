...
async function runningWebsite(
  ...
  const claim = await repository.claimAnalysis(workerId, 60_000, ["website"]);
...
test("Postgres website authentication wait and resume are four-record atomic and exactly claimed", {
  ...
  try {
    const source = await runningWebsite(repository, "atomic");
    ...
    await assert.rejects(repository.waitAnalysisForAuthentication(source.workerId, source.runId, {
      ...waitingInput,
    ...
    await assert.rejects(repository.resumeAnalysisAfterAuthentication(viewer, resume), (error: unknown) =>
      error instanceof RepositoryError && error.code === "FORBIDDEN");
    await assert.rejects(repository.resumeAnalysisAfterAuthentication(editor, {
      ...resume,
    ...
    const consumed = await repository.resumeAnalysisAfterAuthentication(editor, resume);
    assert.equal(consumed.state, "consumed");
    assert.equal(consumed.authenticationEvidenceReference, evidenceReference);
    assert.deepEqual(await repository.resumeAnalysisAfterAuthentication(editor, resume), consumed);
    ...
    const claimed = await repository.claimAnalysis("authentication-postgres-resumed", 60_000, ["website"]);
...
test("Postgres terminal paths and expired reconciliation close authentication state once", {
  ...
  try {
    const cancelled = await runningWebsite(repository, "cancelled");
    ...
    assert.equal((await repository.getWebsiteAuthenticationWait(owner, cancelled.runId))?.state, "cancelled");

    const failed = await runningWebsite(repository, "failed");
    ...
    await repository.resumeAnalysisAfterAuthentication(owner, resumeInput(
      failed, failedReference, `urn:sha256:${"6".repeat(64)}`, "failed",
    ));
    const failedClaim = await repository.claimAnalysis("authentication-postgres-failure", 60_000, ["website"]);
    ...
    assert.equal((await repository.getWebsiteAuthenticationWait(owner, failed.runId))?.state, "failed");

    const expired = await runningWebsite(repository, "expired");
    ...
    assert.equal(await repository.reconcileWorkflows("authentication-postgres-reconciler"), 1);
    assert.equal((await repository.getWebsiteAuthenticationWait(owner, expired.runId))?.state, "expired");
    assert.equal((await repository.getAnalysis(owner, expired.runId)).status, "failed");
    assert.equal((await repository.listWorkflowEvents(owner, expired.runId))
      .filter(({ type }) => type === "task.reconciled").length, 1);
    assert.equal(await repository.reconcileWorkflows("authentication-postgres-reconciler"), 0);
  } finally {
