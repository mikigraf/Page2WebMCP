...
-- 20260901140000 schema-qualified two COALESCE calls as pg_catalog.coalesce(...).
-- COALESCE is a special SQL construct the parser handles directly, not a real
-- pg_catalog function, so the qualified form always failed with
-- "function pg_catalog.coalesce(text[], text[]) does not exist" the moment this
-- function actually ran. Confirmed live: readiness has never reached this
-- function before now. The plain, unqualified form is unaffected by this
-- function's restrictive search_path, since COALESCE is not resolved by schema
-- lookup at all.
...
begin
  ...
  with required_migrations(version) as (
    values
      ...
      ('20260901130000'), ('20260901140000'), ('20260904220000')
