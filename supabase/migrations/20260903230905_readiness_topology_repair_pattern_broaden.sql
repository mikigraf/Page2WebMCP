...
-- Broaden the self-tracking match from this one function family to every
-- post-baseline hotfix, matching this repo's own established '_repair'
-- naming convention (e.g. live_verifier_attestation_v2_repair). Each new
-- hotfix this session needs the same treatment for the same reason: the
-- Supabase migration tool assigns each applied migration's version from its
-- own apply time, not a version its author chooses.
...
begin
  ...
  ), hotfix_migrations(version) as (
    select migration.version::text
    from supabase_migrations.schema_migrations migration
    where migration.name like '%\_repair%' escape '\'
      and migration.version::text > '20260901140000'
  ), required_migrations(version) as (
    select version from baseline_migrations
    union
    select version from hotfix_migrations
  ), migration_state as (
    select
